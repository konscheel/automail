import {suggestFullEmails} from "../dist/index";
...
it('should return full e-mail suggestions instead of just the domain', () => {
    let input = 'username@g'
    let emailSuggestions = suggestFullEmails(input, {domains: knownDomains})
    emailSuggestions.forEach(s =>
        expect(s.startsWith('username@')).toBeTruthy()
    )
});