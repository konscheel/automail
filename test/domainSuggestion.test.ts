import {domainSuggestion} from "../src"
...
it('should start making suggestions when a character was typed after the @-symbol', function () {
    let input = 'username@g'
    let suggestions = domainSuggestion(input, {domains: knownDomains})
...
it('should limit the shown suggestions by the value passed as maxSuggestions',  () => {
    let input = 'username@yahoo'
    let suggestions = domainSuggestion(input, {domains: knownDomains, maxSuggestions: 10})
...
it('should not crash for invalid parameters', () => {
    let input = ['username@yahoo']
    // @ts-ignore
    expect(domainSuggestion(input, {domains: true})).toHaveLength(0)
    // @ts-ignore
    expect(() => domainSuggestion(input, {domains: true})).not.toThrow('user input should be of type string.')
});